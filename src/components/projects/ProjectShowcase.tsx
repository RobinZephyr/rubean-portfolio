...
function ProjectShowcase({ selectedProjectData }: ProjectShowcaseProps) {
  const hoverProjectEnter = (event: React.MouseEvent<HTMLDivElement>) => {
    const projectOverlayHover = event.currentTarget.querySelector(
      ".projectOverlayHover"
    );
    ...
    const projectYoutubeHover = event.currentTarget.querySelector(
      ".projectYoutubeHover"
    );
    ...
    <div className="h-50% md:h-[550px]  relative">
      <div className="projectShowcaseIdle  relative" style={{ zIndex: 100 }}>
        ...
        <div className="z-10 relative border-8 border-t-0 border-skillMenu  bg-skillMenu  mt-5 md:mt-">
          ...
          <div
            className="h-full absolute top-0 left-0 w-full  hover:cursor-pointer"
            onMouseEnter={hoverProjectEnter}
            onMouseLeave={hoverProjectLeave}
          >
            <span
              className="h-full absolute top-0 left-0 w-full flex items-center justify-center opacity-0 projectYoutubeHover"
              style={{ zIndex: "100" }}
            >
              <a
                href={selectedProjectData.link}
                className="flex justify-center"
                title={selectedProjectData.link}
                target="_blank"
              >
                <Image
            ...
            <span className="h-full absolute top-0 left-0 w-full bg-white opacity-0 projectOverlayHover"></span>
          </div>
        </div>
      </div>

      <div
        className="absolute bottom-[-20px] md:bottom-[20px] left-0 w-full flex justify-center projectShadowIdle"
        style={{ zIndex: "5" }}
      >
        <Image
          src={circle}
